function [data, params] = lab5_exp2sim_best(Time,obst_pos) 

    % simulating the experiment using data returned from ground station 

    % input:
    %   Time = time from mocap flight
    %   obst_pos = [3 x $] matrix position of obstacle (object to be
    %   tracked)
    ...
    o_desired = [0; 0; 0];
    % - goal position
    % initial o_goal is location of green sphere in visualize
    o_goal = [0;0;0];
    ...
    params.ue = [0;0;0; params.m*params.g];
        ...
        Rd = eye(4);
  % note that this must be consistent with what is flashed to quadrotor
    params.K = dlqr(Ad,Bd,Qd,Rd);
    
  %  - planner
  % INITIAL EXPERIMENTAL VALUES
    params.k_att = 5; % speed of sphere
    params.b_att = 1;
    params.k_rep = 2;   % pushing away from obstacles
    params.b_rep = 0.4; % convergence to goal
    params.k_des = 0.02;  % step size
    params.b_des = 0.01; % step size 

%   % TEST 2 VALUES '..test5.mp4' (behaves well)
%       params.k_att = 5; % speed of sphere
%       params.b_att = 1;
%       params.k_rep = 2;   % pushing away from obstacles
%       params.b_rep = 0.4; % convergence to goal
%       params.k_des = 0.5;  % step size
%       params.b_des = 0.01; % step size 

%   % TEST 3 VALUES '..test6.mp4'
%       params.k_att = 5; % speed of sphere
%       params.b_att = 1;
%       params.k_rep = 2;   % pushing away from obstacles
%       params.b_rep = 0.4; % convergence to goal
%       params.k_des = 0.5;  % step size
%       params.b_des = 0.4; % step size (WAY TOO BIG)

%     % TEST 4 VALUES '..test7.mp4'
%       params.k_att = 5; % speed of sphere
%       params.b_att = 1;
%       params.k_rep = 2;   % pushing away from obstacles
%       params.b_rep = 0.4; % convergence to goal
%       params.k_des = 0.7;  % step size
%       params.b_des = 0.01; % step size 

%     % TEST 5 VALUES '..test8.mp4'
%       params.k_att = 5; % speed of sphere
%       params.b_att = 1;
%       params.k_rep = 2;   % pushing away from obstacles
%       params.b_rep = 0.4; % convergence to goal
%       params.k_des = 0.7;  % step size
%       params.b_des = 0.02; % step size (0.03 "wobbles")

%     % TEST 6 VALUES '..test9.mp4'
%       params.k_att = 5; % speed of sphere
%       params.b_att = 1;
%       params.k_rep = 2;   % pushing away from obstacles
%       params.b_rep = 0.4; % convergence to goal
...
%       params.b_des = 0.02; % step size (0.03 "wobbles")
    ...
    % TEST 7 VALUES '..test10.mp4'
%       params.k_att = 5; % speed of sphere
%       params.b_att = 1;
%       params.k_rep = 2;   % pushing away from obstacles
%       params.b_rep = 0.4; % convergence to goal
%       params.k_des = 0.2;  % step size
%       params.b_des = 0.02; % step size (0.03 "wobbles")
    ...
    for i = 1:round(tf/params.dt)-1
    %for i = 1:4957
        % Current time and state
        t = data.t(:, end);
        x = data.x(:, end);
        
        %%%%%%%% Read Obst Postion from Experiment %%%%%%%%
        obst{1}.p = obst_pos(:,i);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
        %%%%%%%% TRACKING AN OBJECT %%%%%%%%
        o_goal = obst_pos(:,i) + [0;0;-0.5];
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function obst = ...
    ...
    for i=1:n
        ...
        while(1)
            ...
            s = 0.2;    % radius of sphere of obstacle (defined in groundstation)
